...
import { testPatternBuffer } from "./fixtures.spec";
...
describe("PatternBuffer", function () {
  ...
  it("transposes buffer correctly", async function () {
    ...
    assert.ok(
      JSON.stringify(in2_buffer) == JSON.stringify(in_buffer),
      "something went wrong with transposing the onsets buffer"
    );
    ...
    assert.ok(
      JSON.stringify(in2_buffer) == JSON.stringify(in_buffer),
      "something went wrong with transposing the velocities buffer"
    );
    assert.notStrictEqual(in_buffer, out_buffer);
    ...
    assert.ok(
      JSON.stringify(in2_buffer) == JSON.stringify(in_buffer),
      "something went wrong with transposing the offsets buffer"
    );
    assert.notStrictEqual(in_buffer, out_buffer);
  });
  it("converts buffer to pattern and vice versa", async function () {
    ...
    assert.ok(
      JSON.stringify(patternBuffer.onsetsBuffer) ==
        JSON.stringify(newPatternBuffer.onsetsBuffer)
    );
    assert.ok(
      JSON.stringify(patternBuffer.velocitiesBuffer) ==
        JSON.stringify(newPatternBuffer.velocitiesBuffer)
    );
    assert.ok(
      JSON.stringify(patternBuffer.offsetsBuffer) ==
        JSON.stringify(newPatternBuffer.offsetsBuffer)
    );

    const onsetsBuffer = patternBuffer.onsetsBuffer;
    const velocitiesBuffer = patternBuffer.velocitiesBuffer;
    const offsetsBuffer = patternBuffer.offsetsBuffer;
    newPatternBuffer = new PatternBuffer(
      onsetsBuffer,
      velocitiesBuffer,
      offsetsBuffer
    );
    ...
    for (let i = 0; i < patternBuffer.onsets.length; i++) {
      for (let j = 0; j < patternBuffer.onsets[0].length; j++) {
        assert.ok(
          floatEqual(patternBuffer.onsets[i][j], newPatternBuffer.onsets[i][j])
        );
        assert.ok(
          floatEqual(
            patternBuffer.velocities[i][j],
            newPatternBuffer.velocities[i][j]
          )
        );
        assert.ok(
          floatEqual(
            patternBuffer.offsets[i][j],
            newPatternBuffer.offsets[i][j]
          )
        );
      }
    }
  ...
  it("gets correctly formatted buffer", async function () {
    ...
    const step = 4;
    const onsetsBuffer = Array.from(
      patternBuffer.onsetsBuffer.slice(CHANNELS * step, CHANNELS * (step + 1))
    );
    const velocitiesBuffer = Array.from(
      patternBuffer.velocitiesBuffer.slice(
        CHANNELS * step,
        CHANNELS * (step + 1)
      )
    );
    const offsetsBuffer = Array.from(
      patternBuffer.offsetsBuffer.slice(CHANNELS * step, CHANNELS * (step + 1))
    );

    const testBuffer1 = onsetsBuffer
      .concat(velocitiesBuffer)
      .concat(offsetsBuffer);
    const testBuffer2 = Array.from(
      buffer.slice(CHANNELS * 3 * step, CHANNELS * 3 * (step + 1))
    );
